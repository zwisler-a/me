export default {
  welcome: {
    greeting: 'Hallo, ich bin',
    name: 'Alexander Zwisler',
    nameSubTitle: "Student und Full Stack Developer",
    frontText: `Derzeit studiere ich in Leipzig B.Sc Informatik und bin nebenher als Werksstudent bei der Exxeta Gmbh tätig.`,
    contactMe: 'Contact me'
  },
  about: {
    title: 'Über mich',
    aboutFirst:
      '',
    aboutSecond:
      'Proident cupidatat duis adipisicing excepteur quis qui minim quis cillum ex cupidatat. Ad proident nostrud ex ad amet labore quis cupidatat labore minim cillum velit anim reprehenderit. Ut quis nostrud ipsum est consequat aliquip occaecat consectetur. ',
    aboutTechs1: ['Angular', 'TypeScript', 'NodeJS'],
    aboutTechs2: ['Java', 'Gitlab (CI)', '']
  },
  experience: {
    title: 'Arbeit',
    companies: [
      {
        name: 'EXXETA',
        fullName: 'EXXETA GmbH',
        fromTo: '09.2017 - Bis jetzt',
        title: 'Werksstudent',
        desc: '',
        techs: ['Angular', 'Jenkins', 'NodeJS', 'JavaScript', 'Jasmine & Karma', 'Gitlab', 'Jira', 'Vaadin', 'Java', 'Spring', 'Activi, Camunda, Flowable']
      },
      {
        name: 'ARTEC',
        fullName: 'ARTEC Engineering GmbH',
        fromTo: '10.2016 - 03.2017',
        title: 'Softwareentwickler',
        desc: 'Testen, Dokumentieren und entwickeln einer Heim-Automatisierungs Lösung.',
        techs: ['Jasime & Karma', 'Protractor', 'Angular 2 / AngularJS with ng-metadata', 'C#', 'Jenkins', 'NodeJS']
      },
      {
        name: 'KLEO',
        fullName: 'KLEO Halbleitertechnik GmbH',
        fromTo: '11.2014 - 12.2015',
        title: 'Aushilfe bei der Softwareentwicklung',
        desc: 'Evaluation und Integration einer Bilderkennungs Software für ein LDI-System und Erweiterung und Verbesserung der Benutzeroberfläche',
        techs: ['AngularJS', 'JavaScript', 'Java', 'TwinCAT 2', 'EyeVision']
      }
    ]
  },

  projects: {
    title: 'Projekte',
    featured: 'Projekte',
    features: [
      {
        name: 'Game-Engine',
        image: 'assets/images.jpg',
        desc:
          'Aliquip Lorem minim et consequat nisi nostrud labore quis nostrud voluptate eiusmod reprehenderit. In adipisicing pariatur nisi sint cupidatat adipisicing adipisicing mollit est ea deserunt nulla proident aute.',
        techs: ['Java', 'OpenGL', 'LWJGL', '"Blender"'],
        link: ''
      },
      {
        name: 'Uni Projekt',
        image: 'assets/images.jpg',
        desc:
          'Sint cupidatat voluptate mollit elit magna dolore adipisicing. Qui sit aute adipisicing excepteur ex minim occaecat commodo ad commodo. Eu duis consectetur nulla esse.',
        techs: ['Angular', 'ExpressJS (mit TypeScript)'],
        link: ''
      }
    ],
    github: 'Projekte auf Github'
  }
};
